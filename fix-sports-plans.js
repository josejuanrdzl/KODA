...
async function run() {
    console.log("Adding 'sports' module access to plans...");

    const plans = ['personal', 'team', 'business', 'enterprise'];
    const moduleSlug = 'sports';
    ...
    if (!moduleData) {
        const { error: insertErr } = await supabase.from('modules').insert([{
            slug: moduleSlug,
            name: 'Deportes',
            description: 'Resultados y agenda de partidos deportivos usando ESPN API'
        }]);
        if (insertErr) {
    ...
    console.log("Done configuring sports plan access.");
}
