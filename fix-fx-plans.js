...
async function run() {
    console.log("Adding 'fx-rates' module access to plans...");

    const plans = ['personal', 'team', 'business', 'enterprise'];
    const moduleSlug = 'fx-rates';

    // 1. Ensure the module exists in 'modules' table
    const { data: moduleData } = await supabase.from('modules').select('*').eq('slug', moduleSlug).single();
    if (!moduleData) {
        await supabase.from('modules').insert([{
            slug: moduleSlug,
            name: 'Tipos de Cambio',
            description: 'Consulta de tipos de cambio de divisas'
        }]);
    ...
    for (const p of plans) {
        const { data: pm } = await supabase.from('plan_modules')
            ...
            .eq('plan_slug', p);
        if (!pm || pm.length === 0) {
            await supabase.from('plan_modules').insert([{ plan_slug: p, module_slug: moduleSlug, is_included: true }]);
    ...
    console.log("Done configuring fx-rates plan access.");
}
