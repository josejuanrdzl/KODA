...
function parseActions(text) {
    ...
    while ((match = actionRegex.exec(text)) !== null) {
        ...
        else if (type === 'LUNA_LOG_CYCLE') {
            // Formato: cycle_start|cycle_length|notes
            const lunaParts = argsRaw.split('|');
            if (lunaParts.length >= 1) {
                const cycle_start = lunaParts[0].trim();
                const cycle_length = lunaParts[1] ? parseInt(lunaParts[1].trim()) : 28;
                const notes = lunaParts.slice(2).join('|').trim();
                actions.push({ type, payload: { cycle_start, cycle_length, notes } });
            }
        }
        else if (type === 'LUNA_LOG_SYMPTOM') {
            // Formato: symptom
            const symptom = argsRaw.trim();
            if (symptom) {
