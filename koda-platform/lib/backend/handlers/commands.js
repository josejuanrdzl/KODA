...
const { sendChannelMessage, sendModuleUpsell } = require('../utils/messenger');
...
Registrado el: ${fechaRegistro}`, { parse_mode: 'Markdown' }, channel);
        ...
        case '/journal':
            ...
            if (!hasJournal) {
                await sendModuleUpsell(bot, chatId, channel, 'Diario Personal (Journal)');
                return true;
            }
        ...
        case '/configurar':
            ...
            if (configParam1 === 'proactivo') {
                const hasProactive = await checkModuleAccess(user, 'proactive_messaging');
                if (!hasProactive) {
                    await sendModuleUpsell(bot, chatId, channel, 'Mensajes Proactivos');
                    return true;
                }
                try {
                    if (configParam2 === 'on') {
        ...
        case '/habitos':
            ...
            if (!hasHabits) {
                await sendModuleUpsell(bot, chatId, channel, 'Seguimiento de Hábitos');
                return true;
            }
