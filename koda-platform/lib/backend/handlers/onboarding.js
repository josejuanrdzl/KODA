...
async function handleOnboarding(bot, msg, user) {
    const userId = user.id; // Clave universal primaria en base de datos
...
Para darte el mejor servicio, necesito conocerte un poco. Son solo 3 preguntas rápidas. ¿Empezamos?`, {}, channel);
        onboardingSessions.set(userId, 'waiting_start');
        return;
    }

    const step = onboardingSessions.get(userId);
    ...
    if (step === 'waiting_start') {
        ...
        onboardingSessions.set(userId, 'waiting_name');
    }
...
(Responde con el número)`, {}, channel);
        onboardingSessions.set(userId, 'waiting_tone');
    }
...
(Responde con el número)`, {}, channel);
        onboardingSessions.set(userId, 'waiting_gender');
    }
        ...
        const finalUser = await db.updateUser(user.id, { gender, onboarding_complete: true });
